...
- 增强 rate limit API 表达能力，避免只依赖单一 RPS 语义
- 打磨 retry / backoff / cooldown 语义与限流配合方式
- 增加 host / session 级并发与限速控制的扩展能力
- 为错误体提供安全预览能力，降低生产日志误用风险
- 明确易变 payload 的 `json.RawMessage` 使用策略

### 请求会话与代理基础

- 支持会话级 cookie jar 与基础会话保持能力
- 支持会话粘性代理，保证同一会话尽量固定走同一代理
- 增加代理健康检查、失败评分与临时冷却能力
- 增加代理观测指标，如成功率、延迟、重试次数、拦截率
- 保持官方 API 流量与公开商店页流量在策略上的边界分离

### 商店公开页稳定性能力

- 增加少量稳定的 header profile 预设
- 支持固定、路由式、上下文生成式 `Referer` 策略
- 增加条件请求与短时缓存能力
- 增加 block 检测、429/403 识别与恢复策略
- 为自定义 TLS / 浏览器回退方案预留 transport 扩展点
