...
// Config holds shared smoke-test configuration loaded from live smoke credentials.
type Config struct {
	Key           string
	AccessToken   string
...
func RequireAPIKey(cfg Config) bool {
	...
	fmt.Println("skip: examples/live/key.txt is empty")
	return false
}
...
func RequireAccessToken(cfg Config) bool {
	...
	fmt.Println("skip: examples/live/access-token.txt is empty")
	return false
}
...
func RequireFamilyGroupID(cfg Config) bool {
	...
	fmt.Println("skip: examples/live/family-group-id.txt is empty")
	return false
}
...
func credentialRoots() []string {
	return []string{liveRoot(), legacyTestRoot()}
}
